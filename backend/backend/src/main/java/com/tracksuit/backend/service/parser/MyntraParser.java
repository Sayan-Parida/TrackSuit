...
@Component
public class MyntraParser implements PlatformParser {

    private static final Pattern SUBJECT_PATTERN = Pattern.compile(
            "(?i)(myntra.*order|order.*myntra|shipped|delivered|order\\s+confirmed|your\\s+order)",
            Pattern.CASE_INSENSITIVE
    );

    private static final Pattern PRODUCT_NAME_PATTERN = Pattern.compile(
            "(?i)(?:item|product|ordered|style)[:\\s]+([^\\n]{5,80})",
            Pattern.CASE_INSENSITIVE
    );

    private static final Pattern STATUS_PATTERN = Pattern.compile(
            "(?i)(shipped|delivered|out for delivery|in transit|order placed|dispatched|on its way)",
            Pattern.CASE_INSENSITIVE
    );
    ...
    public boolean canParse(String subject, String body) {
        ...
        return combined.contains("myntra") && SUBJECT_PATTERN.matcher(subject + " " + body).find();
    }
